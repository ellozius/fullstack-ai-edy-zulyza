# 🪜 Milestone 4: Fullstack Integration

## 🎯 Goals
- ID: Integrasi model AI ke aplikasi web/mobile.
- EN: Integrate AI models into web/mobile apps.

---

## 📚 Learning Notes

🧩 Tahap 1: REST API basics  
🧩 Tahap 2: Frontend-backend integration  
🧩 Tahap 3: AI model integration

---

## 🛠️ Portfolio Project
**📊 Proyek | Project:**  
- ID: Aplikasi web dengan integrasi model AI  
- EN: Web application with AI model integration  

**📝 Deskripsi | Description:**  
- ID: Membuat REST API dan frontend sederhana untuk menampilkan hasil prediksi model.  
- EN: Build REST API and simple frontend to display model predictions.  

**🔗 Status | Status:**  
- ID: Belum selesai dan belum terdokumentasi di repo.  
- EN: Not yet completed and not documented in the repository.

---

## 🔍 Challenges & Reflections
📌 ID: Akan diisi bertahap setelah menjalani proses pembelajaran.  
📌 EN: Will be filled gradually after going through the learning process.

---

## ✅ Progress Checklist
- [ ] REST API basics  
- [ ] Frontend-backend integration  
- [ ] AI model integration  

<!-- update commit message milestone-4 -->

---
_Last updated: 2025-11-18_
